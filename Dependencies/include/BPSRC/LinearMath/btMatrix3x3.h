...
operator*(const btVector3& v, const btMatrix3x3& m)
{
	...
	__m128 result = _mm_add_ps(c0, c2);

	float xf, yf, zf;
	_mm_store_ss(&xf, result);
	_mm_store_ss(&yf, _mm_shuffle_ps(result, result, _MM_SHUFFLE(1, 1, 1, 1)));
	_mm_store_ss(&zf, _mm_shuffle_ps(result, result, _MM_SHUFFLE(2, 2, 2, 2)));

	return btVector3(xf, yf, zf);
